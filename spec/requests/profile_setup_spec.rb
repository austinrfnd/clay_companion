  ...
  describe 'PATCH /profile_setup' do
    context 'when user is authenticated' do
      ...
      context 'with valid attributes' do
        let(:photo_file) do
          file_path = Rails.root.join('spec', 'fixtures', 'files', 'test_image.jpg').to_s
        ...
        before do
          ...
          unless File.exist?(file_path)
            ...
            File.binwrite(file_path, jpeg_data)
          end
        end
        ...
        it 'attaches the profile photo' do
          patch profile_setup_path, params: { artist: valid_attributes.merge(profile_photo: photo_file) }
          expect(response).to have_http_status(:redirect) # Ensure update succeeded
          artist.reload
          # Ensure attachment is loaded
          artist.profile_photo.attachment.reload if artist.profile_photo.attached?
